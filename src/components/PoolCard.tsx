"use client";

import { motion } from "framer-motion";
import Link from "next/link";
import { MapPin, Package, DollarSign, Users, Clock, ArrowRight, Camera } from "lucide-react";
import type { PoolStats } from "@/lib/types";
import { SHIPPING_EMOJI, DEFAULT_ORIGIN_LABEL } from "@/lib/constants";
import { setPledgeIntent } from "@/lib/pledge-guide";

interface PoolCardProps {
  pool: PoolStats;
  index?: number;
}

/** Single object from pool_stats view (has both pool fields and stats) */
export function PoolCard({ pool, index = 0 }: PoolCardProps) {
  const thresholdFt3 = (Number(pool.usable_ft3) * Number(pool.announce_threshold_pct)) / 100;
  const isThresholdReached = Number(pool.pct_full) >= Number(pool.announce_threshold_pct);
  const remainingToThreshold = Math.max(
    0,
    thresholdFt3 - Number(pool.total_ft3)
  );

  const statusConfig: Record<string, { label: string; color: string; icon: string }> = {
    collecting: {
      label: "Open for Pledges",
      color: "bg-blue-100 text-blue-700",
      icon: "🟢",
    },
    announced: {
      label: "Announced",
      color: "bg-green-100 text-green-700",
      icon: "🎉",
    },
    loading: {
      label: "Loading",
      color: "bg-amber-100 text-amber-700",
      icon: "📦",
    },
    shipped: {
      label: "Shipped",
      color: "bg-indigo-100 text-indigo-700",
      icon: "🚢",
    },
    arrived_port: {
      label: "At Port",
      color: "bg-cyan-100 text-cyan-700",
      icon: "⚓",
    },
    arrived_destination: {
      label: "In Zambia",
      color: "bg-emerald-100 text-emerald-700",
      icon: "🇿🇲",
    },
    cleared: {
      label: "Cleared",
      color: "bg-teal-100 text-teal-700",
      icon: "✅",
    },
    ready_pickup: {
      label: "Ready for Pickup",
      color: "bg-green-100 text-green-800",
      icon: "🏁",
    },
    closed: {
      label: "Closed",
      color: "bg-gray-100 text-gray-600",
      icon: "🔒",
    },
  };
  const status = statusConfig[pool.status] ?? statusConfig.collecting;
  const isCollecting = pool.status === "collecting";

  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, delay: index * 0.1, ease: [0.4, 0, 0.2, 1] }}
      whileHover={{ y: -4, transition: { duration: 0.2 } }}
      className="group"
    >
      <Link
        href={`/pool/${pool.slug}`}
        onClick={() => isCollecting && setPledgeIntent("from_pool_list")}
      >
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden">
          <div className="p-6 pb-4">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-2">
                <span className="text-2xl">{SHIPPING_EMOJI}</span>
                <h3 className="text-lg font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">
                  {pool.title}
                </h3>
              </div>
              <span
                className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${status.color}`}
              >
                <span>{status.icon}</span>
                {status.label}
              </span>
            </div>
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-gray-500">
              <span className="flex items-center gap-1">
                <MapPin className="w-4 h-4" />
                {pool.origin_region || DEFAULT_ORIGIN_LABEL} → {pool.destination_city}
              </span>
              <span className="text-gray-300">•</span>
              <span>{pool.container_type} container</span>
              {pool.ships_at && (
                <>
                  <span className="text-gray-300">•</span>
                  <span className="text-ocean-700 font-medium">
                    Ship window: {new Date(pool.ships_at + "T12:00:00").toLocaleDateString("en-US", { month: "short", year: "numeric", day: "numeric" })}
                  </span>
                </>
              )}
            </div>
            {(pool.sponsor_name || pool.sponsor_company) && (
              <p className="mt-1.5 text-xs text-gray-400">
                Listed by {[pool.sponsor_company, pool.sponsor_name].filter(Boolean).join(" · ")}
              </p>
            )}
          </div>

          <div className="px-6 py-3 bg-gray-50/50">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700">Container Fill</span>
              <span
                className={`text-sm font-bold ${
                  isThresholdReached ? "text-green-600" : "text-gray-900"
                }`}
              >
                {Number(pool.pct_full).toFixed(0)}% Full
              </span>
            </div>
            <div className="relative h-3 bg-gray-200 rounded-full overflow-hidden">
              <div
                className="absolute top-0 bottom-0 w-0.5 bg-orange-400 z-10"
                style={{ left: `${pool.announce_threshold_pct}%` }}
              />
              <motion.div
                className={`h-full rounded-full ${
                  isThresholdReached ? "bg-green-500" : "bg-blue-500"
                }`}
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(Number(pool.pct_full), 100)}%` }}
                transition={{
                  duration: 1,
                  delay: index * 0.1 + 0.3,
                  ease: "easeOut",
                }}
              />
            </div>
            <div className="flex justify-between mt-1 text-xs text-gray-400">
              <span>0%</span>
              <span className="text-orange-500">🎯 {pool.announce_threshold_pct}% goal</span>
              <span>100%</span>
            </div>
          </div>

          {/* 3-column: stats | stats | container image */}
          <div className="grid grid-cols-2 md:grid-cols-[1fr_1fr_120px] gap-4 p-6 pt-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center">
                <Package className="w-5 h-5 text-blue-600" />
              </div>
              <div>
                <div className="text-lg font-bold text-gray-900">
                  {Number(pool.total_ft3).toLocaleString()}
                </div>
                <div className="text-xs text-gray-500">ft³ pledged</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-green-50 flex items-center justify-center">
                <DollarSign className="w-5 h-5 text-green-600" />
              </div>
              <div>
                <div className="text-lg font-bold text-gray-900">
                  {pool.ship_cost_reach_pct != null
                    ? `${Math.round(Number(pool.ship_cost_reach_pct))}%`
                    : `$${Number(pool.est_revenue).toLocaleString()}`}
                </div>
                <div className="text-xs text-gray-500">
                  {pool.ship_cost_reach_pct != null ? "est. ship cost reach" : "est. revenue"}
                </div>
              </div>
            </div>
            {/* Container image — reality anchor */}
            <div className="hidden md:flex row-span-2 aspect-square rounded-xl border border-gray-200 bg-gray-50 items-center justify-center overflow-hidden">
              {pool.container_image_url ? (
                <img
                  src={pool.container_image_url}
                  alt="Container"
                  className="w-full h-full object-cover rounded-xl"
                />
              ) : (
                <div className="flex flex-col items-center gap-1.5">
                  <Camera className="w-5 h-5 text-gray-300" />
                  <span className="text-[10px] text-gray-400 text-center leading-tight px-1">View Container</span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-purple-50 flex items-center justify-center">
                <Users className="w-5 h-5 text-purple-600" />
              </div>
              <div>
                <div className="text-lg font-bold text-gray-900">{pool.pledge_count}</div>
                <div className="text-xs text-gray-500">pledges</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-orange-50 flex items-center justify-center">
                <Clock className="w-5 h-5 text-orange-600" />
              </div>
              <div>
                <div className="text-lg font-bold text-gray-900">
                  {Number(pool.usable_ft3).toLocaleString()}
                </div>
                <div className="text-xs text-gray-500">ft³ capacity</div>
              </div>
            </div>
          </div>

          <div className="px-6 pb-6">
            {isCollecting && !isThresholdReached && remainingToThreshold > 0 && (
              <div className="mb-3 p-3 bg-orange-50 rounded-lg border border-orange-100">
                <p className="text-sm text-orange-700">
                  <span className="font-semibold">{remainingToThreshold.toFixed(0)} ft³</span> more
                  needed to reach announcement threshold!
                </p>
              </div>
            )}
            <span
              className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-ocean text-white rounded-xl font-medium hover:bg-ocean-800 transition-colors group/btn"
              {...(index === 0 && isCollecting ? { "data-guide": "pool-cta" } : {})}
            >
              {isCollecting ? "Pledge Your Space" : "View Details"}
              <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
            </span>
          </div>
        </div>
      </Link>
    </motion.div>
  );
}

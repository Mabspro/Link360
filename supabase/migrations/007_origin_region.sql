-- Origin region: stage for regional filtering (US nationwide; Phase 2 filter by origin_region + destination_city)
alter table public.pools add column if not exists origin_region text;

comment on column public.pools.origin_region is 'US origin region for pool (e.g. NorCal, Bay Area, Texas, Nationwide). Used for regional discovery later.';

-- Recreate pool_stats to include origin_region
...
group by p.id, p.slug, p.title, p.destination_city, p.container_type, p.usable_ft3, p.announce_threshold_pct, p.status, p.is_public, p.ships_at, p.target_ship_cost, p.sponsor_id, p.origin_region, s.name, s.company;
